...
def generate_random_start(index):
    ...
    # HTML formatting
    r1 = ranked_resumes.Name[0]
    r2 = ranked_resumes.Name[1]
    r3 = ranked_resumes.Name[2]
    r4 = ranked_resumes.Name[3]
    r5 = ranked_resumes.Name[4]

    seed_html = ''
    seed_html = addContent(seed_html, header('Top Resume 1', color='darkblue'))
    seed_html = addContent(seed_html,
                           box(remove_spaces(' '.join(r1))))

    gen_html = ''
    gen_html = addContent(gen_html, header('Top Resume 2', color='darkred'))
    gen_html = addContent(gen_html, box(remove_spaces(' '.join(r2))))

    a_html = ''
    a_html = addContent(a_html, header('Top Resume 3', color='darkgreen'))
    a_html = addContent(a_html, box(remove_spaces(' '.join(r3))))

    seed_html1 = ''
    seed_html1 = addContent(seed_html1, header('Top Resume 4', color='darkblue'))
    seed_html1 = addContent(seed_html1,
                           box(remove_spaces(' '.join(r4))))

    seed_html2 = ''
    seed_html2 = addContent(seed_html2, header('Top Resume 5', color='darkred'))
    seed_html2 = addContent(seed_html2,
                           box(remove_spaces(' '.join(r5))))

    return f'<div>{seed_html}</div><div>{gen_html}</div><div>{a_html}</div><div>{seed_html1}</div><div>{seed_html2}</div>'
